#include <stdio.h>
#include <string.h>
#include <stdint.h>

// fake API-like strings placed in .rodata
const char *apis = "VirtualAlloc\0WriteProcessMemory\0CreateRemoteThread\0GetProcAddress\0LoadLibraryA\0";

// small function that uses inline asm to emit an int 0x80 sequence (linux 32-bit syscall)
int do_syscall_pattern() {
    volatile int res = 0;
    asm volatile (
        "movl $1, %%eax\n\t"   // syscall number (exit) just as pattern
        "int $0x80\n\t"
        : "=a" (res)
        :
        : "memory"
    );
    return res;
}

// high-entropy array to trigger entropy heuristic
unsigned char blob[128] = {
    
};

int main(void) {
    printf("mock injection sample\n");
    do_syscall_pattern();
    // touch blob so compiler doesn't optimize it away
    volatile unsigned char x = blob[0] ^ blob[1];
    return (int)x;
}
