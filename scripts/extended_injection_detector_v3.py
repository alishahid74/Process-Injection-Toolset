#!/usr/bin/env python3
"""
extended_injection_detector_v2.py (patched with improved collect_targets)

Upgraded detector that integrates:
 - PE arch detection via pefile
 - robust section read fallbacks
 - data-section heuristics (entropy, large non-printable runs)
 - PE-in-blob detection (reflective/manual-map indicator)
 - syscall-number extraction
 - ordered API-sequence detection (hollowing, remote injection, section mapping)
 - PEB-walk sequence detection (improved)
 - entrypoint / imagebase write heuristic
 - YARA starter generation for blobs
 - confidence scoring per detection and CSV/HTML outputs
 - arch filtering (--arch-filter all|x86|x64) to limit scanning to a specific architecture
 - improved collect_targets to include ELF/shebang/executable-bit files when scanning directories
"""
import os
import sys
import argparse
...
# default empty syscall map; user can provide JSON to --syscallmap
DEFAULT_SYSCALL_MAP = {}
...
# base confidence per detection type (used to compute row confidence)
BASE_CONFIDENCE = {
    "direct_syscall": 9,
    "int_0x2e": 9,
    "syscall_number": 9,
    ...
    for func in cfg.kb.functions.values():
        ...
        try:
            for block in func.blocks:
                ...
                found_syscall, val, mapped, snippet = inspect_syscall_insn(insns, arch, syscall_map)
                if found_syscall:
...
def collect_targets(path):
    """
    Collect target files from a path. If path is a directory, include:
      - files with common executable extensions (.exe, .dll, .so, .bin)
      - files that start with PE magic (MZ) or ELF magic (\x7fELF) or shebang (#!)
      - files with executable bit set
    If a single file path is provided, return it expanded.
    """
    files = []
...
def filter_targets_by_arch(targets, arch_filter):
    """
    Given a list of file paths, return only those matching arch_filter (x86/x64/all).
    Uses pefile first (fast) and angr loader as a fallback for detection.
    """
    ...
    for t in targets:
        ...
        try:
            ...
            a = None
        if a is None:
            # try a fast angr probe (catch errors)
            try:
